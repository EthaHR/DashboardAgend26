 ...
 * 1. __DIR__ = Raíz/Views/Pages/OperatorsBackend/src/view
...
<link rel="stylesheet" href="Views/Pages/OperatorsBackend/src/styles/modal.css">
<link rel="stylesheet" href="Views/Pages/OperatorsBackend/src/styles/datatable.css">
...
<dialog id="modalConfirmarEliminar">
    <form method="dialog">
        <h3>¿Estás seguro de eliminar el operador?</h3>
...
<dialog id="modalEditar" style="padding: 20px; border-radius: 8px; border: 1px solid #ccc; width: 90%; max-width: 500px;">
    <form method="post" class="form">
        <h3 class="text-center text-secondary" style="margin-top: 0;">Modificar Operador</h3>
        ...
        <div class="grid-form" style="display: grid; grid-template-columns: 1fr; gap: 10px; margin-bottom: 15px;">
            <div class="mb-2">
                ...
                <input type="text" class="form-control" name="nombre_operador" id="edit_nombre_operador" style="width: 100%; padding: 6px; font-size: 18px;">
            </div>
        </div>
        ...
        <div class="col-xs-12">
            <div class="box box-info">
                <div class="box-header with-border">
                    <h3 class="box-title">Lista de Operadores Registrados</h3>
                </div>
                <div class="box-body">
                    ...
                    <?php
                    if ($conexion) {
                        $sql = $conexion->query("SELECT * FROM operador ORDER BY id_operador ASC");
                    } else {
                        die("<div class='alert alert-danger'>Error de conexión.</div>");
                    }
                    ?>
                    <table id="tablaOperadores" class="table table-bordered table-striped table-hover" style="width:100%">
                        <thead>
                            <tr>
                                <th>#</th>
                        ...
                        <tbody>
                            <?php while ($datos = $sql->fetch_object()) { ?>
                                <tr>
                                    <td><?= htmlspecialchars($datos->id_operador) ?></td>
                                    <td><?= htmlspecialchars($datos->nombre_operador) ?></td>
                                    <td><?= $datos->fecha_registro ?></td>
                                    <td>
                                        <button onclick="abrirModalEditar('<?= htmlspecialchars($datos->id_operador, ENT_QUOTES) ?>', '<?= htmlspecialchars($datos->nombre_operador, ENT_QUOTES) ?>', '<?= $datos->fecha_registro ?>')" class="btn btn-xs btn-warning">
                                            <i class="fa fa-pencil"></i>
                                        </button>
                                        <a onclick="event.preventDefault(); eliminar('<?= htmlspecialchars($datos->id_operador, ENT_QUOTES) ?>')" href="#" class="btn btn-xs btn-danger">
...
<script>
    ...
    function eliminar(id) {
        ...
        function handler() {
            if (modal.returnValue === 'confirm') {
                ...
                fetch('Views/Pages/OperatorsBackend/src/controllers/eliminar.php?delete=' + id, {
                        method: 'GET'
                    })
    ...
    function abrirModalEditar(id, nombre_operador, fecha_registro) {
        document.getElementById("edit_id").value = id;
        document.getElementById("edit_nombre_operador").value = nombre_operador;
        ...
        formEditar.onsubmit = function(e) {
            ...
            fetch('Views/Pages/OperatorsBackend/src/controllers/modificar.php', {
                    method: 'POST',
                    body: formData
                })
                .then(response => response.text())
